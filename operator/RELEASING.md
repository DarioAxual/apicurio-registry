...
- Bundles are **immutable** once merged — channel annotations cannot be changed retroactively (without FBC migration or maintainer override labels)
- The `replaces` field must point to a version that exists in the same channel
- `ci.yaml` uses `updateGraph: replaces-mode`
